    ...
    for (const char of text) {
      switch (currentState) {
        case State.bracketOpen:
          if (char === ")" && shouldJumpToEnd(bracketsStk, char)) {
            currentState = State.bracketEnd;
        ...
        case State.text:
          ...
          } else if (char === ")" && shouldJumpToEnd(bracketsStk, char)) {
            currentState = State.bracketEnd;
          }
          if (char === "(") {
            bracketsStk.push("(");
      ...
      ++endIndex;
      if (currentState === State.bracketEnd) {
        break;
      }
    }
  }
...
export function clearConsole(
  ...
  while ((matchText = reg.exec(text))) {
    ...
    const range = new vscode.Range(
      document.positionAt(matchText.index),
